...
for file in filenames:
    ...
    for heading in soup.find_all("div", {"class": ["h5", "h6", "hc", "he", "h7", "h8", "h9", "h10", "h13"]}):
        ...
        # Extract text from image using Tesseract OCR engine
        text_from_image = pytesseract.image_to_string(Image.fromarray(img))

        # Check if the extracted text from image is bold or not
        is_bold = False
        if heading.find("b"):
            is_bold = True
        elif text_from_image.isupper() and text_from_image == text:
            is_bold = True
